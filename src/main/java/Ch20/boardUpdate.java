...
public class boardUpdate extends HttpServlet{
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		int nowPage = Integer.parseInt(req.getParameter("nowPage"));	//현재페이지번호
		int num = Integer.parseInt(req.getParameter("num"));			//보고있는 게시물 번호
		...
		String inpwd = bean.getPass(); //기존저장된 패스워드
		
		BoardMgr bMgr = new BoardMgr();
		if(uppwd.equals(inpwd))
		{
				bMgr.updateBoard(upbean);	//DB수정처리
				resp.sendRedirect("/HTMLCSSJS/Ch20JSP/Read.jsp?nowPage="+nowPage+"&num="+num);
		}
		else
		{
			//패스워드 불일치 
